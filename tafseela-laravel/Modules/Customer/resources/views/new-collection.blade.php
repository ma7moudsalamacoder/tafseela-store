<x-layout.client title="Tafsela | تفصيلة - مجموعة جديدة" :cartCount="$cartCount ?? 2" :wishlistCount="$wishlistCount ?? 0">
                ...
                <nav class="flex justify-end mb-8 text-[11px] font-bold uppercase tracking-widest text-gray-400">
                    <ul class="flex items-center gap-3">
                        ...
                        <li class="flex items-center gap-2"><span
                                ...
                                href="{{ route('collection', $slug) }}">{{ $collection->title }}</a></li>
                    </ul>
                </nav>
                                    ...
                                    @foreach ($activeCategories as $activeCategory)
                                        <li>
                                            <a class="hover:text-[#8B5E3C] flex justify-between items-center transition-colors {{ $categoryId == $activeCategory->id ? 'text-[#8B5E3C]' : '' }}"
                                                href="{{ route('collection', $slug) }}?{{ http_build_query(array_merge(request()->except(['categoryId', 'subcategoryId', 'page']), ['categoryId' => $activeCategory->id])) }}">
                                    ...
                                    @foreach ($subcategories as $subcategory)
                                        <li>
                                            <a class="hover:text-[#8B5E3C] flex justify-between items-center transition-colors {{ $subcategoryId == $subcategory->id ? 'text-[#8B5E3C]' : '' }}"
                                                href="{{ route('collection', $slug) }}?{{ http_build_query(array_merge(request()->except(['subcategoryId', 'page']), ['subcategoryId' => $subcategory->id])) }}">
