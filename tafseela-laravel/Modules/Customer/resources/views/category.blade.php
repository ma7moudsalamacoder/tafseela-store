                ...
                <nav class="flex justify-end mb-8 text-[11px] font-bold uppercase tracking-widest text-gray-400">
                    <ul class="flex items-center gap-3">
                        ...
                        <li class="flex items-center gap-2"><span
                                ...
                                href="{{ route('category', ['slug' => $slug]) }}">{{ $category->category }}</a></li>
                    </ul>
                </nav>
                            ...
                            <div>
                                ...
                                <ul class="space-y-4 text-xs font-bold text-gray-600 dark:text-gray-400">
                                    @foreach ($subcategories as $subcategory)
                                        <li>
                                            <a class="hover:text-[#8B5E3C] flex justify-between items-center transition-colors {{ $selectedSubcategory && $selectedSubcategory->id === $subcategory->id ? 'text-[#8B5E3C]' : '' }}"
                                                href="{{ route('category', ['slug' => $slug]) }}?{{ http_build_query(array_merge(request()->except(['subcategoryId', 'page']), ['subcategoryId' => $subcategory->id])) }}">
                                                <span>{{ $subcategory->title }}</span>
                                                <span
                                                    class="font-sans text-[10px] {{ $selectedSubcategory && $selectedSubcategory->id === $subcategory->id ? '' : 'text-gray-300' }}">({{ $subcategory->products_count }})</span>
                                            </a>
                                        </li>
                                    @endforeach
                                </ul>
                    ...
                    <div class="flex-grow">
                        <div
                            ...
                            <h2 class="text-2xl font-extrabold font-display tracking-tight">
                                {{ $selectedSubcategory->title ?? 'المنتجات' }} <span
                                    class="text-gray-300 font-light text-sm mr-4">({{ $selectedSubcategory->products_count ?? 0 }}
                                    منتج)</span></h2>
