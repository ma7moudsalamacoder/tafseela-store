<x-layout.client title="Tafsela | تفصيلة - المتجر" :cartCount="$cartCount ?? 2" :wishlistCount="$wishlistCount ?? 0">
                                    ...
                                    @foreach ($activeCategories as $activeCategory)
                                        <li>
                                            <a class="hover:text-[#8B5E3C] flex justify-between items-center transition-colors {{ $categoryId == $activeCategory->id ? 'text-[#8B5E3C]' : '' }}"
                                                href="{{ route('shop') }}?{{ http_build_query(array_merge(request()->except(['categoryId', 'subcategoryId', 'page']), ['categoryId' => $activeCategory->id])) }}">
                                    ...
                                    @foreach ($subcategories as $subcategory)
                                        <li>
                                            <a class="hover:text-[#8B5E3C] flex justify-between items-center transition-colors {{ $subcategoryId == $subcategory->id ? 'text-[#8B5E3C]' : '' }}"
                                                href="{{ route('shop') }}?{{ http_build_query(array_merge(request()->except(['subcategoryId', 'page']), ['subcategoryId' => $subcategory->id])) }}">
