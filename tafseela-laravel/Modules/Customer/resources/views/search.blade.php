<x-layout.client title="Tafsela | تفصيلة - البحث" :cartCount="$cartCount ?? 2" :wishlistCount="$wishlistCount ?? 0">
                ...
                <nav class="flex justify-end mb-8 text-[11px] font-bold uppercase tracking-widest text-gray-400">
                    <ul class="flex items-center gap-3">
                        ...
                        <li class="flex items-center gap-2"><span
                                class="material-symbols-outlined text-sm">chevron_left</span> <span
                                class="hover:text-primary transition-colors">بحث: {{ $keyword }}</span></li>
                    </ul>
                </nav>
                                    ...
                                    @foreach ($activeCategories as $activeCategory)
                                        <li>
                                            <a class="hover:text-[#8B5E3C] flex justify-between items-center transition-colors {{ $categoryId == $activeCategory->id ? 'text-[#8B5E3C]' : '' }}"
                                                href="{{ route('search') }}?{{ http_build_query(array_merge(request()->except(['categoryId', 'subcategoryId', 'page']), ['categoryId' => $activeCategory->id])) }}">
                                    ...
                                    @foreach ($subcategories as $subcategory)
                                        <li>
                                            <a class="hover:text-[#8B5E3C] flex justify-between items-center transition-colors {{ $subcategoryId == $subcategory->id ? 'text-[#8B5E3C]' : '' }}"
                                                href="{{ route('search') }}?{{ http_build_query(array_merge(request()->except(['subcategoryId', 'page']), ['subcategoryId' => $subcategory->id])) }}">
                    ...
                    <div class="flex-grow">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-x-8 gap-y-16">
                            @if ($formattedProducts->isEmpty())
                                <p class="text-center col-span-full text-gray-500">لا توجد نتائج لبحثك.</p>
                            @else
